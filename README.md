...
这台设备上唯一被验证过**真的能写进去**的只有 plus.io 的文本写入，所以：

- **照片**存成 base64 文本文件：`_doc/food/food_<时间>.b64`
- **完整备份**是**一个普通的 JSON 文本文件**（`calorie-backup-<日期>-full.json`），
  记录和每张照片的 base64 都在里面 —— 换手机时拷这一个文件就行
- 两者都**不经过 zip、不经过 Native.js、不经过 `copyTo`**
- 导出位置若在应用私有目录（`Android/data/...`，Android 11+ 文件管理器看不到），
  界面上会直接给一个**「分享文件」**按钮，把它发到微信/网盘
- 导入时会**额外扫描手机公共的 Download / Documents**（微信、数据线进来的文件在那儿），
  逐个候选路径试着列目录，能列出内容的才算数

代价是照片文件比原图大约 1/3，换来的是"确实存在、而且能当场核对"。

写入失败绝不会被伪装成成功：照片存不下时记录的图片字段留空，
导出时也会如实报告"有几张照片文件已经不在了"。
